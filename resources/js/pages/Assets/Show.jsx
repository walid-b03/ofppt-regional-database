...
const STATE_COLORS = {
    ...
    'Inactif': 'bg-stone-100 text-stone-600',
    'Endommagé': 'bg-red-50 text-red-700',
    'Perdu': 'bg-amber-50 text-amber-700',
};
...
export default function Show({ asset }) {
    ...
    return (
        <Dashboard title={`Actif — ${asset.name}`}>
            <Head title={`${asset.name} — OFPPT`} />
            ...
            <div className="mx-auto max-w-5xl space-y-6">
                ...
                {/* Hero */}
                <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-cyan-700 via-cyan-600 to-blue-600 shadow-lg">
                    ...
                    <div className="relative px-6 py-10 sm:px-10 sm:py-14">
                        <div className="flex flex-col items-start gap-4 sm:flex-row sm:items-center">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-cyan-200">{asset.code}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
                    <UserInfoCard
                        ...
                        items={[
                            ...
                            ['Type', asset.type],
                            ['État', asset.state ? (
                                <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${STATE_COLORS[asset.state] || ''}`}>
                                    {asset.state}
                                </span>
                            ) : null],
                            ['Établissement', asset.establishment?.name],
                        ]}
                    />
                    {asset.description && (
                        <UserInfoCard title="Description" icon="mail" items={[['Description', asset.description]]} />
                    )}
                    {asset.notes && (
                        <UserInfoCard title="Notes" icon="mail" items={[['Notes', asset.notes]]} />
                    )}
                </div>
            </div>
        </Dashboard>
